#!/usr/bin/env python3
"""Full-graph rank64 fixed-edge CPU/MPS gradient preflight on idle macm3.

One forward/backward per device, zero optimizer steps, no test-set access.
All existing rank128, model and trainer sources remain unchanged.
"""
import argparse
...
ARM = "G32rank64fixed"
CONFIG = {"d_embed": 32, "readout_rank": 64, "history_length": 8,
          "lag_map": list(range(8)), "seed": 42, "plasticity": "fixed"}
COUNTS = {"embedding": 32768, "input_projection": 450048, "neurons": 148179,
          "edge_gains": 0, "layernorm": 98786, "readout": 3226688, "other": 0, "total": 3956469}
READOUT_SHAPES = {"lm_head.0.weight": [64, 49393], "lm_head.1.weight": [1024, 64]}
require = shared.require


def source_receipt():
    return {**shared.source_receipt(), "scripts/audit_connectorch_rank64.py": legacy.file_hash(Path(__file__))}
...
def verify_initialization(cpu_parameters, mps_parameters):
    ...
    require(set(cpu_parameters) == set(mps_parameters) == shared.COMMON_PARAMETERS,
            "Rank64 initialization parameter names differ")
    for name, expected_shape in READOUT_SHAPES.items():
        require(list(cpu_parameters[name].shape) == expected_shape, "Rank64 factor shape differs: " + name)
    ...
    require(sum(value.numel() for value in cpu_parameters.values()) == COUNTS["total"], "Rank64 initial parameter count differs")
    return {"passed": True, "cpu_mps_parameter_hashes_equal": True,
            "parameter_hashes": hashes, "readout_shapes": READOUT_SHAPES,
            "qualification": "Seed42 from scratch; rank64 second-factor std remains0.02, so initial logit scale differs from rank128."}


def verify_gradients(record, tensors):
    import torch
    gradients = tensors["gradients"]
    require(set(gradients) == shared.COMMON_PARAMETERS, "Missing or extra rank64 gradient names")
    require(record.get("passed") is True and record["parameter_count"] == COUNTS["total"],
            "Rank64 base audit or parameter count failed")
    checks = {}
    for name, value in gradients.items():
        if name in READOUT_SHAPES:
            require(list(value.shape) == READOUT_SHAPES[name], "Rank64 factor gradient shape differs: " + name)
        finite, nonzero = bool(torch.isfinite(value).all()), int(value.count_nonzero())
        require(finite and nonzero > 0, "Rank64 required gradient is zero or nonfinite: " + name)
...
def main():
    ...
    result = {"format_version": 1, "passed": False, "started_at": datetime.now(timezone.utc).isoformat(),
              ...
              "scope": "G32rank64fixed full-graph CPU/MPS forward/backward parity; no optimizer, no F completion prerequisite.",
    ...
    try:
        ...
        require(trainer.parameter_counts(cpu) == COUNTS
                ...
                "Rank64 parameter partition differs")
        ...
        require(result["passed"], "Rank64 numerical parity or binding gate failed")
