#!/usr/bin/env python3
"""Inspect G's plateau; --execute stops its exact dispatcher and preserves state.
...
RUN_RELATIVE = "results/connectorch-rank64-v1"
ARM_NAME = "G32rank64fixed"
LAUNCH_ID = "bd29bc14-f3a1-4971-b6bf-fed447a7622d"
MANIFEST_SHA256 = "412bbd505baf6da2be16f603d01dde865f0fc7bd65fd917f0656e8da764b5df0"
CONTROLLER, TRAINER = 20942, 20985
CONTROLLER_BIRTH = "Tue Sep 15 19:09:07 2026"
TRAINER_BIRTH = "Tue Sep 15 19:09:21 2026"
...
def verify_identity(run, arm, launch, process):
    ...
    require("run_connectorch_rank64_campaign.py" in launch["command"][2]
            and LAUNCH_ID in launch["command"] and str(run) in launch["command"], "Unexpected controller command")
    command = process["command"]
    require(str(arm) in command and command[command.index("--readout-rank") + 1] == "64"
            and command[command.index("--d-embed") + 1] == "32"
            and command[command.index("--plasticity") + 1] == "fixed", "Unexpected G trainer architecture")
    ...
    require(state.get("status") == "running" and state.get("phase") == "training"
            ...
            "Campaign is not running the expected G training job")
...
def safe_interval(status, last_update):
    return (status.get("status") == "running" and status.get("pid") == TRAINER and status.get("activity") == "training"
...
def load_and_verify(path, recorded, manifest, expected_sha=None, entry=None, selector=None, latest_update=None):
    ...
    require(sum(value.numel() for value in saved["parameters"].values()) == 3956469
            and all(value.device.type == "cpu" and value.dtype == torch.float32 for value in saved["parameters"].values())
            and list(saved["parameters"]["lm_head.0.weight"].shape) == [64, 49393]
            and list(saved["parameters"]["lm_head.1.weight"].shape) == [1024, 64], "Unexpected checkpoint parameter architecture")
...
def main():
    ...
    require(c.file_hash(run / "manifest.json") == MANIFEST_SHA256, "Bound rank64 campaign manifest changed")
    ...
    expected = {"d_embed": 32, "readout_rank": 64, "history_length": 8, "plasticity": "fixed", "seed": 42,
    ...
    require(all(recorded["config"].get(key) == value for key, value in expected.items())
            and recorded["parameter_counts"]["total"] == 3956469 and recorded.get("debug") is False,
            "G recorded configuration differs")
    import train_connectorch as trainer
    ...
    if not safe_interval(observed, rows[-1]["updates"]):
        ...
        return
    evidence = plateau(rows, observed)
    decision = {"format_version": 1, "arm": ARM_NAME, "decision": "stop_plateau_for_rank64_assessment",
    ...
    receipt = {"format_version": 1, "status": "accepted_early_stop", "arm": ARM_NAME, "training_host": "macm3",
               ...
               "comparison_policy": "Compare E rank128 and G rank64 at common recorded budgets and both retained winners; total training budgets differ.",
               "launch_id": LAUNCH_ID, "campaign_manifest_sha256": MANIFEST_SHA256}
