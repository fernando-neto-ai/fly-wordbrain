...
def main():
    ...
    report = ["# Connectorch encoder experiment — partial validation", "",
              ...
              "The exact ngxson reference must finish successfully before the M3 preflight and four serial runs.",
              ...
              "| Arm | Width | Edge gains | Status | Updates | Best validation CE | Accuracy at best CE |",
              "|---|---:|---|---|---:|---:|---:|"]
    histories = {}
    for arm in ARMS:
        prefix = "arms/" + arm + "/"
        arm_status = read(prefix + "status.json")
        ...
        for i, line in enumerate(lines):
            ...
            if row.get("event") == "validation":
                rows.append(row)
        histories[arm] = rows
        best = min(rows, key=lambda r: r["validation"]["cross_entropy"]) if rows else None
        ce = f"{best['validation']['cross_entropy']:.4f}" if best else "—"
        accuracy = f"{100*best['validation']['top1_accuracy']:.2f}%" if best else "—"
        report.append(f"| {arm} | {128 if '128' in arm else 32} | {'fixed' if 'fixed' in arm else 'bounded10'} | "
                      f"{arm_status.get('status', 'pending')} | {arm_status.get('updates', 0):,} | {ce} | {accuracy} |")
    if read("failure.json"):
        report.extend(["", "Campaign failure: `" + json.dumps(read("failure.json")) + "`"])
    for arm, rows in histories.items():
        if not rows:
            continue
        report.extend(["", "## " + arm, "", "| Updates | Epoch | Validation CE | Accuracy |",
                       "|---:|---:|---:|---:|"])
        for row in rows:
