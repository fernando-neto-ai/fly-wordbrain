#!/usr/bin/env python3
"""Fetch rank-64 G progress beside the preserved rank-128 E baseline."""
import argparse
...
BASELINE = 'E32rank128fixed'
ARM = 'G32rank64fixed'
DEFAULT_REMOTE_RUN = '/Users/fernando/fly_wordbrain_connectorch/results/connectorch-rank64-v1'
...
COUNTS = {
    BASELINE: {'encoder':482816, 'readout':6453376, 'neurons':148179, 'layernorm':98786, 'edge_gains':0, 'total':7183157},
    ARM: {'encoder':482816, 'readout':3226688, 'neurons':148179, 'layernorm':98786, 'edge_gains':0, 'total':3956469},
}
...
if manifest:
 ...
 if baseline.name!=request['baseline'] or Path(entry['path']).resolve()!=baseline/'accepted-early-stop.json':
  raise ValueError('Expected the preserved E baseline')
 ...
 if parent:
  ...
  if parent_manifest.name!='manifest.json' or baseline!=parent_manifest.parent/'arms'/request['baseline']:
   raise ValueError('Rank64 parent does not contain the E baseline')
  ...
  if json.loads(files['parent/manifest.json']['text']).get('launch_id')!=parent.get('launch_id'):
   raise ValueError('Rank64 parent launch identity differs')
 ...
 if receipt.get('arm')!=request['baseline'] or receipt.get('status')!='accepted_early_stop' or receipt.get('process_cessation',{}).get('confirmed') is not True or receipt.get('accepted_by') not in ('user','agent_under_standing_user_authorization'):
  raise ValueError('E baseline is not a confirmed accepted stop')
 ...
 if Path(stop['path']).resolve()!=baseline/'stop-receipt.json':raise ValueError('Unexpected E stop receipt path')
 ...
 if prefix+'accepted-early-stop.json' in files:
  ...
  if receipt.get('status')=='accepted_early_stop':
   if receipt.get('arm')!=request['arm'] or receipt.get('process_cessation',{}).get('confirmed') is not True:
    raise ValueError('Unconfirmed G stop receipt')
   stop=receipt['stop_receipt']
   if Path(stop['path']).resolve()!=root/prefix/'stop-receipt.json':raise ValueError('Unexpected G stop receipt path')
...
def arm_summary(files, name, prefix, branch_mapping):
    ...
    rank = 128 if name == BASELINE else 64
    ...
    if config and any(config.get(key) != value for key,value in expected_config.items()):
        raise ValueError('Unexpected E/G architecture: '+name)
    ...
    if raw:
        ...
        if actual != counts or sum(value for key,value in raw.items() if key != 'total') != raw.get('total'):
            raise ValueError('Unexpected E/G parameter counts: '+name)
    ...
    if not stopped and (failure or result.get('status') == 'failed'):
        state = 'failed'
...
def summarize_snapshot(files, host, now):
    ...
    return {'format_version':1, 'snapshot_at_utc':now, 'host':host,
            ...
            'arms':{ARM:arm_summary(files,ARM,'arms/'+ARM+'/',mapping)},
...
def render_report(summary):
    ...
    report = ['# Rank-64 decoder experiment — partial validation','',
              ...
              'G compares a rank-64 readout with the preserved rank-128 E baseline. Encoder capacity stays at 482,816 parameters; '
              'the readout falls from 6,453,376 to 3,226,688 parameters. Both use fixed canonical edges, 148,179 trained neuron parameters and 98,786 output LayerNorm parameters.',
    ...
    if not summary['baseline']['accepted_stop']:
        report.extend(['','E acceptance has not yet been fetched; baseline results remain pending.'])
    report.extend(['','E stopped early. Compare common recorded updates as well as retained-best results across actual budgets. '
    ...
    if summary['smokes']:
        report.extend(['',f"G smoke receipt passed: **{summary['smokes'].get('passed','unavailable')}**."])
    if summary['failure']:
...
def main():
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'results/connectorch-rank64-v1')
    ...
    if not snapshot['files']:
        raise RuntimeError('No rank64 campaign artifacts found')
    hashes={}
