...
spec = importlib.util.spec_from_file_location("rank32_plateau_stop", ROOT / "scripts/stop_connectorch_rank32_plateau.py")
stop = importlib.util.module_from_spec(spec)
spec.loader.exec_module(stop)


@pytest.fixture(autouse=True)
def synthetic_trainer_identity(monkeypatch):
    monkeypatch.setattr(stop, "TRAINER", 12345)


def test_pending_native_bindings_fail_before_process_or_host_calls(monkeypatch):
    monkeypatch.setattr(stop, "LAUNCH_ID", None)
    with pytest.raises(ValueError, match="native launch bindings are pending"):
        stop.require_launch_bound()


def test_unbound_trainer_cannot_match_missing_pid(monkeypatch):
    monkeypatch.setattr(stop, "TRAINER", None)
    assert not stop.safe_interval({"status": "running", "activity": "training", "updates": 13020, "batch": 10}, 13000)
...
@pytest.mark.parametrize("change,last_update", [({"activity": "validation"}, 13000),
    ...
    ({"pid": 12346}, 13000), ({"batch": 115}, 13000),
    ({}, 12900), ({}, 13100)])
