...
spec = importlib.util.spec_from_file_location("rank32_campaign", ROOT / "scripts/run_connectorch_rank32_campaign.py")
...
def parity_fixture(manifest):
    name = d.ARMS[0][0]
    source = "scripts/audit_connectorch_rank32.py"
    manifest["sources_sha256"][str(Path(manifest["root"]) / source)] = "test-source"
    case = {"passed": True, "targets": 64, "parameter_count": 2343125,
...
def test_single_h_exact_rank32_counts_and_recipe(tmp_path):
    assert d.ARMS == (("H32rank32fixed", 32, "fixed"),)
    manifest = manifest_fixture(tmp_path)
    counts = d.expected_counts(d.ARMS[0])
    assert counts["total"] == 2343125 and counts["readout"] == 1613344 and counts["edge_gains"] == 0
    for smoke in (False, True):
        command = d.trainer_command(manifest, d.ARMS[0], tmp_path / "G", smoke)
        assert command[command.index("--readout-rank") + 1] == "32"
...
def worker_fixture(tmp_path, monkeypatch):
    ...
    parity["source_sha256"] = {"scripts/audit_connectorch_rank32.py": manifest["sources_sha256"][str(tmp_path / "scripts/audit_connectorch_rank32.py")]}
...
def test_worker_runs_only_h_smoke_then_h_full(tmp_path, monkeypatch, fail_smoke):
    ...
    assert calls == ([("smoke", "H32rank32fixed")] if fail_smoke else [("smoke", "H32rank32fixed"), ("training", "H32rank32fixed")])
...
# Tiny synthetic checkpoint contents verify acceptance without inference or training.
h = d
trainer_spec = importlib.util.spec_from_file_location("rank32_fixture_parameter_audit", ROOT / "scripts/train_connectorch.py")
...
def accepted_fixture(tmp_path, monkeypatch):
    ...
    directory = parent_dir / "arms" / h.BASELINE_ARM[0]
    directory.mkdir(parents=True)
    graph = {"brain.w_values": "fixture-graph"}
    source = {str(ROOT / name): h.c.file_hash(ROOT / name) for name in h.g.SOURCE_FILES}
    mapping = {"repository": "https://github.com/fernando-neto-ai/fly-wordbrain",
               "arms": {arm[0]: {"branch": "exp/" + arm[0], "commit": "a" * 40} for arm in h.g.ARMS}}
    parent = {"root": str(ROOT), "output": str(parent_dir), "python": "fixture-python", "launch_id": "parent-id",
              ...
              "validation_stories": 100, "validation_tokens": 21874,
              "arms": [{"name": arm[0], **h.g.arm_config(arm), "parameter_counts": h.g.expected_counts(arm)} for arm in h.g.ARMS]}
    initial = {"fixture.weight": torch.zeros(2)}
    recorded = {"config": h.g.arm_config(h.BASELINE_ARM), "debug": False, "parameter_counts": h.g.expected_counts(h.BASELINE_ARM),
    ...
    write(directory / "manifest.json", recorded)
    write(directory / "launch.json", {"campaign_launch_id": "parent-id", "experiment_git": {"repository": mapping["repository"], **mapping["arms"][h.BASELINE_ARM[0]]}})
    ...
    cessation = {"confirmed": True, "processes": [{"pid": pid, "alive": False, "birth": "prior birth"} for pid in (123456, 123450)]}
    write(directory / "stop-receipt.json", {"process_cessation": cessation})
    write(parent_dir / "manifest.json", parent)
    write(parent_dir / "launch.json", {"worker_pid": 123450, "launch_id": parent["launch_id"], "manifest_sha256": h.c.file_hash(parent_dir / "manifest.json")})
    receipt = {"training_host": "macm3", "planned_updates": sum(h.PLANNED_UPDATES), "launch_id": parent["launch_id"],
               "campaign_manifest_sha256": h.c.file_hash(parent_dir / "manifest.json"), "format_version": 1, "status": "accepted_early_stop", "arm": h.BASELINE_ARM[0],
               ...
               "artifact_sha256": {str(directory / name): h.c.file_hash(directory / name) for name in ("manifest.json", "launch.json", "metrics.jsonl", "selected-checkpoints.json", "status.json", "process-status.json")},
...
def test_accepted_stop_binds_both_winners_latest_and_actual_checkpoint_contents(tmp_path, monkeypatch):
    ...
    summary, bindings = h.accepted_g(parent, directory, receipt_path)
    ...
    assert h.c.read_json(directory / "arms" / h.BASELINE_ARM[0] / "status.json")["status"] == "running"


@pytest.mark.parametrize("damage", ["missing_acceptance", "missing_selector", "latest_cursor", "winner_epoch", "artifact", "graph", "omitted_worker", "test", "completed", "wrong_parent", "omitted_dispatcher", "missing_status_binding"])
def test_incoherent_or_unaccepted_stop_is_rejected(tmp_path, monkeypatch, damage):
    ...
    arm = directory / "arms" / h.BASELINE_ARM[0]
    ...
    if damage == "wrong_parent": receipt["campaign_manifest_sha256"] = "0" * 64
    if damage == "missing_status_binding": receipt["artifact_sha256"].pop(str(arm / "status.json"))
    if damage == "omitted_dispatcher":
        receipt["process_cessation"]["processes"] = receipt["process_cessation"]["processes"][:1]
        write(arm / "stop-receipt.json", {"process_cessation": receipt["process_cessation"]})
        receipt["stop_receipt"] = entry(arm / "stop-receipt.json")
    write(receipt_path, receipt)
    with pytest.raises(h.c.CampaignError):
        h.accepted_g(parent, directory, receipt_path)
...
def test_checkpoint_parameter_corruption_rejected_even_with_updated_file_receipt(tmp_path, monkeypatch):
    ...
    (directory / "arms" / h.BASELINE_ARM[0] / "latest.pt").write_bytes(path.read_bytes())
    write(receipt_path, receipt)
