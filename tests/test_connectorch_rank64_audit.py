"""Rank64 preflight contracts using CPU tensors; no model forward or optimizer."""
import copy
...
spec = importlib.util.spec_from_file_location("rank64_audit", ROOT / "scripts/audit_connectorch_rank64.py")
...
def parameters():
    shapes = {"brain.wte.weight": (1024, 32), "brain.in_proj": (8, 32, 1758),
              ...
              "lm_head.0.weight": (64, 49393), "lm_head.1.weight": (1024, 64)}
    return {name: torch.ones(shape, dtype=torch.float32) for name, shape in shapes.items()}


def test_rank64_counts_and_initial_parameters_match_exactly():
    first = parameters()
    ...
    assert sum(v.numel() for v in first.values()) == 3956469
    assert first["lm_head.0.weight"].numel() + first["lm_head.1.weight"].numel() == 3226688
...
def test_rank64_rejects_wrong_factor_shape_and_adaptive_parameters():
    first = parameters()
...
def test_both_readout_factors_require_nonzero_gradients(factor):
    values = parameters()
    record = {"passed": True, "parameter_count": 3956469}
...
def test_nonfinite_input_gradient_cannot_pass():
    ...
    with pytest.raises(ValueError, match="brain.in_proj"):
        audit.verify_gradients({"passed": True, "parameter_count": 3956469}, {"gradients": values})
