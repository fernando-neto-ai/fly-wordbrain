...
The [delayed-memory training protocol](RETENTION_PROTOCOL.md) tests whether
supervising earlier-word identity and order at the final prediction improves
retention. It trains the existing 8,838 plasticity parameters and two small
linear heads (3,084 parameters), against a matched decoder-only control. Both
arms train with fast weights enabled. Validation selects checkpoints before
held-out tests and fast-state ablations; this measures synthetic memory, not
next-word accuracy. Its separate dashboard is generated by
`scripts/report_retention.py` from partial or completed run artifacts.
The [first retention report](RETENTION_REPORT.md) records the completed
256-update comparison and its validation-selected step-0 result.
