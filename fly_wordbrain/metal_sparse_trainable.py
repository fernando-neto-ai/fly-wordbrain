...
def _compiled_library():
    ...
    if _LIBRARY is None:
        if not torch.backends.mps.is_available() or not hasattr(torch.mps, "compile_shader"):
            raise RuntimeError("TrainableCSR on MPS requires torch.mps.compile_shader and Apple GPU hardware; no CPU fallback")
        with _COMPILE_LOCK:
            if _LIBRARY is None:
...
def _index_array(value, name):
    if isinstance(value, torch.Tensor):
        if value.requires_grad:
            raise ValueError(f"Topology cannot require gradients: {name}")
    ...
    if value.size and (value.min() < 0 or value.max() >= np.iinfo(np.int32).max):
        raise ValueError(f"{name} entries must be nonnegative and below 2^31-1")
...
class TrainableCSR:
    ...
    def __init__(self, ptr, indices, shape, device="cpu"):
        ...
        if device.type not in ("cpu", "mps"):
            raise ValueError("TrainableCSR supports only CPU and MPS")
        if (len(shape) != 2 or any(isinstance(x, (bool, np.bool_)) or not isinstance(x, (int, np.integer))
                                  or x < 1 or x >= np.iinfo(np.int32).max for x in shape)):
            raise ValueError("shape must contain two positive integer dimensions below 2^31-1")
        self.shape = torch.Size(map(int, shape))
        rows, columns = self.shape
        ptr, indices = _index_array(ptr, "ptr"), _index_array(indices, "indices")
        if (len(ptr) != rows + 1 or ptr[0] != 0 or ptr[-1] != len(indices)
                or np.any(np.diff(ptr) < 0)):
            raise ValueError("CSR pointer lengths, endpoints, or monotonicity are inconsistent")
        ...
        if self.nnz > 1 and np.any((edge_rows[order[1:]] == edge_rows[order[:-1]])
                                  & (indices[order[1:]] == indices[order[:-1]])):
            raise ValueError("Duplicate edges are unsupported; topology must not be silently coalesced")
        transpose_edges = np.argsort(indices, kind="stable")
        transpose_ptr = np.r_[0, np.cumsum(np.bincount(indices, minlength=columns))]
        arrays = {"ptr": ptr, "col": indices, "row": edge_rows,
                  "transpose_ptr": transpose_ptr, "transpose_col": edge_rows[transpose_edges],
                  "transpose_edge": transpose_edges, "shape": np.array(self.shape)}
        if device.type == "cpu":
        ...
        self._buffers = {name: torch.tensor(value, dtype=dtype, device=device) for name, value in arrays.items()}
        self._identities = {name: (id(value), value._version) for name, value in self._buffers.items()}
    ...
    def _check_cache(self):
        if (self.shape != self._shape_at_construction or self.nnz != self._nnz_at_construction
                or self._buffers.keys() != self._identities.keys()
                or any((id(value), value._version) != self._identities[name]
    ...
    def metadata(self):
        return {"backend": "metal_trainable_csr" if self.device.type == "mps" else "cpu_trainable_csr",
                "shape": list(self.shape), "edges": self.nnz, "source_sha256": METAL_SOURCE_SHA256,
                "topology_sha256": self._fingerprint, "edge_order": "supplied CSR order",
                "edge_weight_gradients": True, "first_order_only": True,
                "dense_adjacency": False, "edge_batch_intermediate": False,
                "per_step_host_transfers": False, "host_fallback": False,
    ...
    def mm(self, h, values):
        ...
        if not isinstance(h, torch.Tensor) or not isinstance(values, torch.Tensor):
            raise TypeError("h and values must be torch tensors")
        if h.layout != torch.strided or values.layout != torch.strided:
            raise ValueError("h and values must have strided dense layout")
        if h.device != self.device or values.device != self.device:
            raise ValueError("h and values must be resident on the topology cache device")
        supported = (torch.float32,) if self.device.type == "mps" else (torch.float32, torch.float64)
        if h.dtype not in supported or values.dtype != h.dtype:
            raise ValueError("h and values must have matching float32 dtype (CPU also accepts float64)")
        if h.ndim != 2 or h.shape[1] != self.shape[1] or values.shape != (self.nnz,):
            raise ValueError("Expected h[batch, columns] and values[edges]")
        # The shaders index outputs with uint32, including their SIMD lanes.
        if h.shape[0] * max(self.shape) * 32 >= 2**32:
            raise ValueError("Batch and graph dimensions exceed Metal dispatch indexing limits")
        return _TrainableMM.apply(h, values, self)

    def _multiply(self, h, values, transpose=False):
        batch = h.shape[0]
        ...
        if self.device.type == "cpu":
            ...
            matrix = torch.sparse_csr_tensor(ptr, col, values[edge],
                                             size=tuple(reversed(self.shape)) if transpose else self.shape,
                                             check_invariants=False)  # Fully validated at construction.
        ...
        if transpose:
            _compiled_library().dynamic_csr_transpose(
                output, h.contiguous(), buffers["transpose_ptr"], buffers["transpose_col"],
                buffers["transpose_edge"], values.contiguous(), buffers["shape"],
                threads=int(batch * rows * 32), group_size=128)
        else:
            _compiled_library().dynamic_csr_forward(
                output, h.contiguous(), buffers["ptr"], buffers["col"], values.contiguous(), buffers["shape"],
                threads=int(batch * rows * 32), group_size=128)
        self.kernel_launch_count["state_backward" if transpose else "forward"] += 1
        return output

    def _edge_gradient(self, h, grad_output):
        ...
        _compiled_library().dynamic_csr_edge_backward(
            result, h.contiguous(), grad_output.contiguous(), buffers["row"], buffers["col"],
            buffers["shape"], int(h.shape[0]), threads=self.nnz, group_size=128)
...
class _TrainableMM(torch.autograd.Function):
    @staticmethod
    def forward(ctx, h, values, graph):
        ctx.graph = graph
    ...
    def backward(ctx, grad_output):
        if torch.is_grad_enabled():
            raise RuntimeError("TrainableCSR supports first-order gradients only; create_graph/higher-order derivatives are unsupported")
        ctx.graph._check_cache()
        h, values = ctx.saved_tensors
        dh = ctx.graph._multiply(grad_output, values, transpose=True) if ctx.needs_input_grad[0] else None
